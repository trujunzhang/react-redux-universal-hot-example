 ...
 * read the message... ;)
 */
const linkPattern = /^(https?|ftp):\/\/([a-zA-Z0-9.-]+(:[a-zA-Z0-9.&%$-]+)*@)*((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])){3}|([a-zA-Z0-9-]+\.)*[a-zA-Z0-9-]+\.(com|edu|gov|int|mil|net|org|biz|arpa|info|name|pro|aero|coop|museum|[a-zA-Z]{2}))(:[0-9]+)*(\/($|[a-zA-Z0-9.,?'\\+&%$#=~_-]+))*$/;
...
export default function fieldValidation(state, action) {
  ...
  switch (field) {
    ...
    case ('link'): {
      let validLink = _.isUndefined(validate({link: value}, linkConstraints));
      if (validLink) {
        return state.setIn(['form', 'fields', 'linkHasError'], false)
          .setIn(['form', 'fields', 'linkErrorMsg'], '');
      } else {
        return state.setIn(['form', 'fields', 'linkHasError'], true)
          .setIn(['form', 'fields', 'linkErrorMsg'],
            'the url address is invalidate');
      }
    }
